  ...
  console.log(userInfo)
    return (
      ...
      <div className="bg-dark" style={{ width: "100%", overflow: "auto" }}>
          ...
          <div className='p-3'>
          <h1 className='mb-3 mt-3 p-1 text-white'>Remove User</h1>
    ...
    <td>
       <button onClick={()=>{
        const deleteuser = async()=> {
          const {data}= await axios.delete(`https://limitless-savannah-08553.herokuapp.com/delete-user?user=${user.email}`)
          if(data.acknowledged){
            const res = userInfo.filter(no=>no.email !== user.email)
            setuserInfo(res)
          }
        }
        deleteuser()
